...
##Orginal Script https://gallery.technet.microsoft.com/SharePoint-User-Profile-928b39c0/
##Updated by:NaemO
##Version: 1.4
...
##1.4 Fixed the Dirsync folder and helper files creation, Save the credential of User Profile Import account in Windows Credential Manager as generic credential
...
{
    Write-Host "Dirsync folder is already exist" -ForegroundColor Cyan
}
else
{
    md $Location
    Write-Host "Dirsync folder is successfully created" -ForegroundColor Green
}
...
    $bckupTime=(Get-Date).ToString("ddMMyyyy-hhmm_")
    $fileName=$file.Name
    $newName=$bckupTime+$file.Name
    Rename-Item $file -NewName $newName
    Write-Host "$fileName is already exist and backuped it to $newName " -ForegroundColor Yellow
}
    New-Item -Path $filePath
    Write-Host "$fileName is successfully created" -ForegroundColor Green
    write-host ('-'*100) -ForegroundColor DarkYellow
}

#Do not change below this line
$cookiepath = $Location+"cookie.bin"
CreateFile -filePath $cookiepath

$log = $Location+"out.log"
CreateFile -filePath $log

$fileloc = $Location+"DNLookup.xml"
CreateFile -filePath $fileloc
...
$startTime=get-date
"Started the script at $startTime" | out-file $log -Append -noclobber
...
##Save the credentin of the account in Windows Credential Manager as generic credential
$cred=(Get-StoredCredential -Target ADUserProfileAcc).GetNetworkCredential()
}
}
...
write-host "Starting to Update the profile photo store to be compatible with SharePoint Server"
Update-SPProfilePhotoStore -MySiteHostLocation $site -CreateThumbnailsForImportedPhotos $true
write-host "Finsihed Updating the profile photo store to be compatible with SharePoint Server"
